...
class App extends Component {
  ...
  loadMap = () => {
    loadScript(
      "https://maps.googleapis.com/maps/api/js?key=AIzaSyDnrGYtkdccFXqQGTNEfousldIW7TdltQM&callback=initMap"
    );
  ...
  initMap = () => {
    ...
    this.state.locations.forEach(location => {
      let marker = new window.google.maps.Marker({
        ...
        location: location.venue.location.address
  ...
  handleClick = location => {
    this.state.markers.forEach(marker => {
      if (
        (marker.venue && marker.venue.name === location) ||
        marker.title === location.venue.name
      ) {
